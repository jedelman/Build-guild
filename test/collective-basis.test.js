...
const prop = (a, action, enacts, basis, t) => vr(a, { type: "org.buildguild.proposal", guild: GUILD, action, enacts, basis, createdAt: t });
...
test("a vote pinning a stale head does not count; recast under the new head does", async () => {
  ...
  // P1 admits C — built on (and A & B vote under) the GENESIS head.
  const p1 = await prop(A, "admit", { grantee: C }, ch._ref, T(2));
  ...
  // P2 grants X a mandate, built causally on the post-admit head p1. Electorate is now
  // {A,B,C}=3 (quorum 50 → need 2 votes). A votes under the STALE genesis head; B votes
  // under the new head p1.
  const p2 = await prop(A, "grant_mandate", { grantee: X, capability: "foo", scope: GUILD }, p1._ref, T(3));
